...
def index():
    ...
    for transaction in l_all:
        if transaction["symbol"] not in owned_shares:
            ...
            if transaction["action"]=="buy":
                properties[i]+=transaction["shares"]
            else:
                properties[i]-=transaction["shares"]
    print(owned_shares, properties)
    return redirect("/")
...
def quote():
    ...
    if request.method== "POST" :
        ...
        if not symbol:
            ...
            print(looked)
            if looked == None:
                return render_template("quoted.html",looked=[None,None,None])
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure password was submitted
            ...
            return apology("password and confirmation should match", 403)
        else:
